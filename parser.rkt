...
(define (scanner i)
  (cond
    ...
    [(equal? (first (list i)) #\() (cons 'Lparen (scanner (rest i)))]
    [(equal? (first (list i)) #\)) (cons 'Rparen (scanner (rest i)))]
    [(equal? (first (list i)) #\+) (cons 'Plus (scanner (rest i)))]
    [(equal? (first (list i)) #\*) (cons 'Multiply (scanner (rest i)))]
    [(equal? (first (list i)) #\-) (cons 'Minus (scanner (rest i)))]
    [(equal? (first (list i)) #\/) (cons 'Divide (scanner (rest i)))]
